document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (currentIndex !== -1) {
      findAndDisplayNextVideo(currentIndex);
    }
  } catch (error) {
    console.error("Error fetching video IDs:", error);
  }

  // Function to find the next video and update the UI
  async function findAndDisplayNextVideo(currentIndex) {
    const nextIndex = currentIndex + 1 < videoIds.length ? currentIndex + 1 : 0;
    const nextVideoId = videoIds[nextIndex];

    try {
      const response = await fetch(`/api/videos/${nextVideoId}`);
      const data = await response.json();

      if (data && data.video) {
        const nextThumbnail = document.getElementById("next-video-thumbnail");
        const nextTitle = document.getElementById("next-video-title");
        const nextViews = document.getElementById("next-video-views");

        nextThumbnail.src = data.video.thumbnail;
        nextTitle.textContent = data.video.title;
        nextViews.textContent = `${data.video.views} views`;
      }
    } catch (error) {
  ...
  if (videoId) {
    try {
      ...
      if (data && data.video) {
        ...
        videoViewsElement.textContent = `${data.video.views} views`;
